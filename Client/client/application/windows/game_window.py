...
class GameWindow(BattleshipWindow):
    ...
    def update_enemy_grid(self, data: dict):
        self.grid_enemy.grid.update_grid_state(data["_x"], data["_y"], GridCellState(data["status"]))

    def update_ally_grid(self, data: dict):
        self.grid_ally.grid.update_grid_state(data["_x"], data["_y"], GridCellState(data["status"]))
    ...
    def send_hit(self):
        ...
        # need to get pos of cell that was clicked
